        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins" style="margin-bottom: 0;">
                ...
                <div class="ibox-content">
                    ...
                    <form role="form" action="{{ route('admin.emails.update', $email->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-lg-3" style="margin-bottom: 15px;">
                                ...
                                <select name="name" id="name" class="form-control">
                                    ...
                                    <option value="user-registration" {{ old('name', $email->name) == 'user-registration' ? 'selected' : '' }}>User Registration</option>
                                    <option value="email-verification" {{ old('name', $email->name) == 'email-verification' ? 'selected' : '' }}>Email Verification</option>
                                    <option value="user-suspend" {{ old('name', $email->name) == 'user-suspend' ? 'selected' : '' }}>User Suspend</option>
                                    <option value="user-active" {{ old('name', $email->name) == 'user-active' ? 'selected' : '' }}>User Active</option>
                                    <option value="user-update" {{ old('name', $email->name) == 'user-update' ? 'selected' : '' }}>User Update</option>
                                    <option value="password-reset" {{ old('name', $email->name) == 'password-reset' ? 'selected' : '' }}>Password Reset</option>
                                    <option value="password-changed" {{ old('name', $email->name) == 'password-changed' ? 'selected' : '' }}>Password Changed</option>
                                    <option value="fees-paid" {{ old('name', $email->name) == 'fees-paid' ? 'selected' : '' }}>Fees Paid</option>
                                    <option value="send-invoice" {{ old('name', $email->name) == 'send-invoice' ? 'selected' : '' }}>Send Invoice</option>
                            ...
                            <div class="col-lg-9" style="margin-bottom: 15px;">
                                ...
                                <input type="text" name="subject" id="subject" class="form-control" value="{{ old('subject', $email->subject) }}">
                                @error('subject')
                                    <p class="text-danger text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="col-lg-12" style="margin-bottom: 15px;">
                                ...
                                <input name="cc" id="cc" value="{{ old('cc', $email->cc) }}" class="form-control" />
                                @error('cc')
                                    <p class="text-danger text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="col-lg-12" style="margin-bottom: 15px;">
                                ...
                                <input name="bcc" id="bcc" value="{{ old('bcc', $email->bcc) }}" class="form-control" />
                                @error('bcc')
                                    <p class="text-danger text-xs italic">{{ $message }}</p>
                                @enderror
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            function updateInstruction() {
                if (emailSelect.value === 'user-registration') {
                    ...
                    instructionText.innerHTML = "Folder email placeholders:<br> <strong>{name}</strong> <strong>{email}</strong> <strong>{folder_name}</strong> <strong>{course_name}</strong> <strong>{head_of_faculty_name}</strong> <strong>{trainer_name}</strong> <strong>{instructor_name}</strong> <strong>{validity}</strong> <strong>{access_till}</strong> <strong>{issued_at}</strong> <strong>{portal_url}</strong> <strong>{login_url}</strong>";
                } else if (emailSelect.value === 'study-material-student-disabled') {
                    instructionText.innerHTML = "Sent when student access is disabled or a folder is disabled/deleted:<br> <strong>{name}</strong> <strong>{email}</strong> <strong>{folder_name}</strong> <strong>{course_name}</strong> <strong>{head_of_faculty_name}</strong> <strong>{trainer_name}</strong> <strong>{instructor_name}</strong> <strong>{reason}</strong> <strong>{portal_url}</strong> <strong>{login_url}</strong>";

...
@section('content')
    ...
    @foreach ($page->sections as $section)
        @if ($section->section_type === 'hero-banner')
            <x-hero-banner background="{{ media_url($section->data['image'] ?? null) }}"
        ...
        @elseif ($section->section_type === 'schools')
            ...
            @php
                $locations = $section->schoolDetails->map(function ($school) {
                    return [
                        'image' => media_url($school->image),
                        'icon' => media_url($school->icon),
        ...
        @elseif ($section->section_type === 'media-section')
            <x-media-section :alignment="$section->data['alignment'] ?? 'left'" :direction="$section->data['image_placement']" :image="media_url($section->data['image'] ?? null)" :icon="$section->data['icon'] ?? null" :title="$section->data['title']" :description="demote_page_headings($section->data['description'] ?? '')"
        ...
        @elseif ($section->section_type === 'cards')
            ...
            @endphp
            <x-cards :columns="$section->data['columns'] ?? '3'" :layout="$section->data['layout'] ?? 'layout-1'" :backgroundColor="$section->data['background'] ?? 'transparent'" :backgroundImage="media_url($section->data['image'] ?? null)"
                :backgroundImageAlt="image_alt($section->data['image_alt'] ?? null, $section->data['title'] ?? 'Section background')"
        ...
        @elseif ($section->section_type === 'clients')
            @php
                $client = $section->clientDetails->map(function ($client) {
                    return [
                        'image' => ! empty($client->image)
                            ? media_url(str_contains((string) $client->image, '/') ? $client->image : 'images/clients/' . $client->image)
                            : null,
                        'url' => $client->url ?? '#',
